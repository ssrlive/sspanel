...
<body {if $user->is_dark_mode === 1}data-bs-theme="dark" {elseif $user->is_dark_mode === 2}data-bs-theme="auto" {/if}>
    <div class="page">
        ...
        <div class="page-wrapper">
                ...
                <div class="page-header d-print-none text-white">
                    <div class="row align-items-center">
                        <div class="col">
                            <h2 class="page-title">
                                <span
                                    class="home-title my-3">{trans key='admin.invoice.invoice_number' id=$invoice->id}</span>
                            </h2>
                            <div class="page-pretitle">
                                <span class="home-subtitle">{trans key='admin.invoice.detail_title'}</span>
                            </div>
                        </div>
                            ...
                            <div class="col-auto">
                                <div class="btn-list">
                                    <button href="#" class="btn btn-primary" data-bs-toggle="modal"
                                        ...
                                        {trans key='admin.invoice.mark_paid'}
                                    </button>
                                </div>
                            </div>
                        {/if}
                ...
                <div class="container-xl">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">{trans key='admin.invoice.basic_info'}</h3>
                        </div>
                        <div class="card-body">
                            <div class="datagrid">
                                <div class="datagrid-item">
                                    <div class="datagrid-title">{trans key='admin.invoice.submitting_user'}</div>
                                    <div class="datagrid-content">{$invoice->user_id}</div>
                                </div>
                                <div class="datagrid-item">
                                    <div class="datagrid-title">{trans key='admin.invoice.related_order_id'}</div>
                                    <div class="datagrid-content">{$invoice->order_id}</div>
                                </div>
                                <div class="datagrid-item">
                                    <div class="datagrid-title">{trans key='admin.invoice.amount'}</div>
                                    <div class="datagrid-content">{$invoice->price}</div>
                                </div>
                                <div class="datagrid-item">
                                    <div class="datagrid-title">{trans key='admin.invoice.status'}</div>
                                    <div class="datagrid-content">{$invoice->status_text}</div>
                                </div>
                                <div class="datagrid-item">
                                    <div class="datagrid-title">{trans key='admin.invoice.created_at'}</div>
                                    <div class="datagrid-content">{$invoice->create_time}</div>
                                </div>
                                <div class="datagrid-item">
                                    <div class="datagrid-title">{trans key='admin.invoice.updated_at'}</div>
                                    <div class="datagrid-content">{$invoice->update_time}</div>
                                </div>
                                <div class="datagrid-item">
                                    <div class="datagrid-title">{trans key='admin.invoice.paid_at'}</div>
                                    <div class="datagrid-content">{$invoice->pay_time}</div>
                                </div>
                                {if $invoice->status === 'paid_gateway'}
                                    <div class="datagrid-item">
                                        <div class="datagrid-title">{trans key='admin.invoice.gateway_trade_number'}</div>
                    ...
                    <div class="card my-3">
                        <div class="card-header">
                            <h3 class="card-title">{trans key='admin.invoice.content_title'}</h3>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table id="invoice_content_table" class="table table-vcenter card-table">
                                    <thead>
                                        <tr>
                                            <th>{trans key='admin.invoice.name'}</th>
                                            <th>{trans key='admin.invoice.price'}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
            ...
            <div class="modal modal-blur fade" id="mark_paid_confirm_dialog" tabindex="-1" role="dialog"
                aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">{trans key='admin.invoice.mark_paid_title'}</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="mb-3">
                                <p>
                                    {trans key='admin.invoice.confirm_mark_paid'}
                                <p>
                            </div>
                        </div>
