...
final class RegController extends BaseController
{
    ...
    public function index(ServerRequest $request, Response $response, array $args): ResponseInterface
    {
        ...
        return $response->write($view->fetch('admin/setting/reg.tpl'));
    }
    ...
    public function save(ServerRequest $request, Response $response, array $args): ResponseInterface
    {
        foreach ($this->update_field as $item) {
            if (! Config::set($item, $request->getParam($item))) {
                return $response->withJson([
                    'ret' => 0,
                    'msg' => I18n::trans('admin.registration.messages.save_failed', $this->user->locale, ['field' => $item]),
                ]);
            }
        }
        ...
        return $response->withJson([
            'ret' => 1,
            'msg' => I18n::trans('admin.registration.messages.saved', $this->user->locale),
        ]);
    }
}
